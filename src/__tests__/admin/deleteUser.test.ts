...
describe("Admin Module", () => {
  ...
  describe("DELETE /api/v1/admin/user/:userId", () => {
    ...
    it("should return a bad request error when no data is deleted from the database and using an admin profile access token", async () => {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      jwt.verify.mockImplementation((_token, _secretOrPublicKey, _options, callback) => {
        callback(null, {
    ...
    it("should return a bad request message when no data is deleted from the database and using an admin profile access token", async () => {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      jwt.verify.mockImplementation((_token, _secretOrPublicKey, _options, callback) => {
        callback(null, {
    ...
    it("should return a conflict error message in case an user tries to delete its own user and using an admin profile access token", async () => {
    ...
    it("should return a successful delete response if the user is deleted from the database and using an admin profile access token", async () => {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      jwt.verify.mockImplementation((_token, _secretOrPublicKey, _options, callback) => {
        callback(null, {
