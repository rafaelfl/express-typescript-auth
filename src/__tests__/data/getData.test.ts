...
describe("Data Module", () => {
  ...
  describe("GET /api/v1/data/:dataId", () => {
    ...
    it("should return an error message if an error occurs when accessing the database", async () => {
      ...
      mockingoose(airbnbDataModel).toReturn(new Error("Error accessing the database"), "findOne");
    ...
    it("should return successful message if the informed dataId exists in the database", async () => {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      jwt.verify.mockImplementation((_token, _secretOrPublicKey, _options, callback) => {
        callback(null, {
      ...
      mockingoose(airbnbDataModel).toReturn(airbnbMockDataInput[0], "findOne");
    ...
    it("should return a not found message if the informed dataId doesn't exists in the database", async () => {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      jwt.verify.mockImplementation((_token, _secretOrPublicKey, _options, callback) => {
        callback(null, {
      ...
      mockingoose(airbnbDataModel).toReturn(null, "findOne");

      await request(app)
        .get("/api/v1/data/10006546")
        .set({ Authorization: `Bearer ${ACCESS_TOKEN}`, Accept: "application/json" })
        .expect(404, { success: false, message: "Unable to retrieve data" });
